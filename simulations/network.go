...
func (net *Network) connect(oneID, otherID enode.ID) error {
	...
	if err == nil {
		one := net.Nodes[net.nodeMap[oneID]]
		other := net.Nodes[net.nodeMap[otherID]]
		one.conns++
		other.conns++
	}
	return err
...
func (net *Network) Disconnect(oneID, otherID enode.ID) error {
	...
	err = client.Call(nil, "admin_removePeer", string(conn.other.Addr()))
	if err == nil {
		one := net.Nodes[net.nodeMap[oneID]]
		other := net.Nodes[net.nodeMap[otherID]]
		one.conns--
...
func (n *Node) NodeInfoWithConns() *SimNodeInfo {
	// avoid a panic if the node is not started yet
	if n.Node == nil {
		return nil
	}
